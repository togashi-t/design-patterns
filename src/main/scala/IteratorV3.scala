...
object IteratorV3 extends App {
  ...
  class CafeMenu extends Menu {
    // コンストラクタ
    val menuItems = new util.HashMap[String, MenuItem]
    addItem("Veggie Burger and Air Fries", "Veggie burger on a whole wheat bun, lettuce, tomato, and fries", true, 3.99)
    addItem("Soup of the day", "A cup of the soup of the day, with a side salad", false, 3.69)
    addItem("Burrito", "A large burrito, with whole pinto beans, salsa, guacamole", true, 4.29)

    def addItem(name: String, description: String, vegetarian: Boolean, price: Double) =
      menuItems.put(name, new MenuItem(name, description, vegetarian, price))

    def createIterator: util.Iterator[MenuItem] = menuItems.values.iterator
  }
